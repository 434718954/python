...
def feature_selection_ig(class_df_list, term_set, term_class_df_mat):
    ...
    p_t = term_df_array / N
    p_not_t = 1 - p_t
    p_c_t_mat =  (A + 1) / (A + B + class_set_size) #避免有些词在某些分类中没有出现过而概率为0的情况，采用加1平滑法，
    ...
    sorted_term_score_index = term_score_array.argsort()[: : -1] #按term_score_array升序排列，然后倒序，因为计算公式中忽略掉-号，其实也就是得出降序后的索引位置
    term_set_fs = [term_set[index] for index in sorted_term_score_index]    
